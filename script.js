  ...
  class Game {
    ...
    setupInput() {
      ...
      this.canvas.addEventListener('touchend', (e)=>{ this.touch.active=false; this.handleTouchEnd(); });
    ...
    createTrashItem() {
      ...
      const size = 50 + Math.random()*30; // 50-80 px
      const symbols = {
        'organic':['🍎','🍌','🍂'],
        'anorganik':['🥤','📄','🥡'],
        'residu':['🧪','🗑️','💡']
    ...
    checkTrashDisposal(trashItem){
      ...
      // if dropped somewhere else, just remove
    ...
    render(){
      ...
      for (const item of this.trashItems){
        ...
        // Draw trash symbol (larger font for better visibility)
        this.ctx.fillStyle = 'white';
        this.ctx.font = '20px Arial'; // chosen size from your snippet
        this.ctx.textAlign = 'center';
        this.ctx.textBaseline = 'middle';
        this.ctx.fillText(item.symbol, item.x + item.width/2, item.y + item.height/2);
      }

      // draw dragging item on top (mouse)
      if (this.mouse.dragging){
